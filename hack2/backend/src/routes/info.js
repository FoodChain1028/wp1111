...
exports.GetSearch = async (req, res) => {
    ...
    if (mealFilter.length !== 0 && typeFilter.length !== 0){
        search = mealFilter.mealFilter.concat(typeFilter.typeFilter)
    }
    // console.log(search)
    // TODO Part I-3-a: find the information to all restaurants
    Info.find({price: priceFilter.priceFilter, tag: {$all: search }}).exec((err, data)=>{
        if (err) {
